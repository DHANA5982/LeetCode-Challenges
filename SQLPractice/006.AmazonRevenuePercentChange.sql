...
-- Amazon: Given a table 'sf_transactions' of purchases by date.
...
CREATE TABLE Amazon_purchase_transactions (
    id INT,
    created_at DATE,
    value INT,
    purchase_id INT
...
INSERT INTO Amazon_purchase_transactions VALUES 
...
SELECT * FROM Amazon_purchase_transactions;

WITH MonthlyRevenue AS (
    SELECT FORMAT(created_at, 'yyyy-MM') AS month_year,
            SUM(value) total_revenue
    FROM Amazon_purchase_transactions
    GROUP BY FORMAT(created_at, 'yyyy-MM')
),
RevenueChange AS (
    SELECT month_year,
            total_revenue,
            LAG(total_revenue) OVER (ORDER BY month_year) previous_revenue
    FROM MonthlyRevenue
)
SELECT month_year, 
        total_revenue,
